...
import (
	...
	"github.com/dappsteros-io/DappsterOS-LocalStorage/pkg/utils/merge"
...
func (s *LocalStorage) SetMerge(ctx echo.Context) error {
	...
	if m.SourceVolumeUuids != nil {
		...
		for _, volumeUUID := range *m.SourceVolumeUuids {
			...
			if !volumeFound {
				message := "volume " + volumeUUID + " not found, or it is not a DappsterOS storage. Consider adding it to DappsterOS first."
				return ctx.JSON(http.StatusBadRequest, codegen.ResponseBadRequest{Message: &message})
			}
		}
	}
...
func (s *LocalStorage) InitMerge(ctx echo.Context) error {
	...
	if strings.ToLower(config.ServerInfo.EnableMergerFS) != "true" {
		if !file.CheckNotExist(m.MountPoint) {
			...
			if len(dir) > 0 {
				message := "Please make sure the /var/lib/dappsteros/files directory is empty"
				return ctx.JSON(http.StatusBadRequest, codegen.ResponseBadRequest{Message: &message})
			}
			...
			if err != nil {
				fmt.Println(err)
				message := "move " + m.MountPoint + " to /var/lib/dappsterosrosros/files failed"
				return ctx.JSON(http.StatusBadRequest, codegen.ResponseBadRequest{Message: &message})
			}
		}
