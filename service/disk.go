...
import (
	...
	command2 "github.com/dappsteros-io/DappsterOS-Common/utils/command"
	"github.com/dappsteros-io/DappsterOS-Common/utils/constants"
	"github.com/dappsteros-io/DappsterOS-Common/utils/exec"
	...
	"github.com/dappsteros-io/DappsterOS-LocalStorage/pkg/partition"
...
const (
	...
	PersistentTypeDappsterOS = "dappsteros"
)
...
func (d *diskService) GetPersistentTypeByUUID(uuid string) string {
	...
	if result := d.db.Where(&model2.Volume{UUID: uuid}).Limit(1).Find(&m); result.Error != nil {
		...
		return PersistentTypeDappsterOS
	}
	...
	for _, v := range diskList {
		if IsDiskSupported(v) {
			if _, ok := diskMap[v.Serial]; !ok {
				...
				// add UI properties to applicable events so that DappsterOS UI can render it
